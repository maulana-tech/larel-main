 ...
 *
 * When a user burns a bridged note on Soroban (`LarelBridge.bridge_out`), the
 ...
 * the hackathon — BRIDGE_SPEC §4/§8) calls `LarelBridgeL1.unlock(commitment,
 ...
 * `unlock`/`locks` ABI are pinned by `bridge/l1/src/LarelBridge.sol`.
 */
import {
  getAddress,
  parseAbiItem,
...
/** Minimal ABI for the L1 lock contract (matches LarelBridge.sol). */
...
/** Call `LarelBridgeL1.unlock(commitment, to)` with the governor wallet. Returns the tx hash. */
...
/** Topic symbol the LarelBridge `bridge_out` event is published under (configurable). */
...
/**
 * Poll the Soroban RPC for `LarelBridge` `bridge_out` events and invoke
