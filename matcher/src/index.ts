/**
 * `@larel/matcher` — off-chain order-matching service for the Larel ZK dark pool.
 *
...
export async function main(): Promise<void> {
  ...
  const deploymentsPath =
    process.env.LAREL_DEPLOYMENTS ?? fileURLToPath(new URL("../../deployments.json", import.meta.url));
  ...
  const mode = process.env.LAREL_SUBMIT === "live" ? "live" : "dry-run";
  ...
  if (circuitPath) {
    const { NoirProver } = await import("@larel/sdk");
  ...
  if (mode === "live") {
    const sourceSecret = process.env.LAREL_MATCHER_SECRET;
    if (!sourceSecret) throw new Error("LAREL_SUBMIT=live requires LAREL_MATCHER_SECRET (a funded S… key)");
    live = { rpcUrl: process.env.LAREL_RPC_URL ?? "https://soroban-testnet.stellar.org", sourceSecret };
  }
