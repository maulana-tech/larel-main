 ...
 */
import { LarelContract, encodePublicInputs, isValidProofLength, type ProofData } from "@larel/sdk";
...
export interface ContractIdSources {
  ...
  contractId?: string;
  /** Environment (defaults to `process.env`); reads `LAREL_POOL_CONTRACT`. */
...
export function resolveContractId(sources: ContractIdSources = {}): string {
  ...
  const id =
    sources.contractId ??
    env.LAREL_POOL_CONTRACT ??
    c?.laxStellPoolMatchMemo?.contract ?? // the pool the frontend targets (match memos)
    c?.laxStellPoolMemo?.contract ??
    c?.laxStellPool;
  if (!id) {
    throw new Error(
      "no LarelPool contract id; set it explicitly, via LAREL_POOL_CONTRACT, or in deployments.json (contracts.laxStellPool)",
    );
  }
  return id;
}
...
export class MatchSubmitter {
  ...
  private readonly contract: LarelContract;
