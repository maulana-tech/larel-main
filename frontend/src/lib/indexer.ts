 ...
 * spendable via {@link LarelIndexer.witnessFor} — this supersedes the deposit-time frontier
...
export class LarelIndexer {
  ...
  async sync(server: rpc.Server, latestLedger: number, floor: number): Promise<SyncStats> {
    ...
    const fetchEvents = async (startLedger: number, pageCursor?: string) => {
      try {
        return await server.getEvents(
          pageCursor ? { filters, cursor: pageCursor, limit: 200 } : { filters, startLedger, limit: 200 },
        )
      } catch (err: any) {
        if (err && err.message && err.message.includes('startLedger must be within the ledger range')) {
          const match = err.message.match(/range:\s*(\d+)\s*-\s*(\d+)/)
          if (match && match[1]) {
            const minLedger = Number(match[1])
            console.warn(`Clamping indexer startLedger from ${startLedger} to RPC minimum ${minLedger}`)
            return await server.getEvents(
              pageCursor ? { filters, cursor: pageCursor, limit: 200 } : { filters, startLedger: minLedger, limit: 200 },
            )
          }
        }
        throw err
    ...
    for (let page = 0; page < 200; page += 1) {
      const res = await fetchEvents(start, cursor)
      stats.pages += 1
