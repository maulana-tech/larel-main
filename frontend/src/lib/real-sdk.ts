/**
 * RealLarelSdk — the live client that talks to the deployed LarelPool on Stellar
 * Testnet, backed by `@larel/sdk` (crypto + Soroban op building), `@stellar/stellar-sdk`
 * (RPC submit) and the Stellar Wallets Kit (multi-wallet address + signing).
 *
 * It implements the same `LarelSdk` surface the UI is written against (see
 * `larel-sdk.ts`), so it drops in behind `createLarelSdk()` with no UI changes.
 *
 * Flow status:
...
import { ASSET_CONFIG, NATIVE_SAC, NETWORK_PASSPHRASE, POOL_CONTRACT_ID, SOROBAN_RPC_URL, SWAP_ROUTER_CONTRACT_ID, SOROSWAP_ROUTER_ADDRESS } from './config'
...
export class RealLarelSdk implements LarelSdk {
  private readonly contract = new LarelContract({
  ...
  async getTransactionHistory(): Promise<HistoryItem[]> {
    ...
    const notesByTx: Record<string, StoredNote[]> = {}
    for (const note of notes) {
      if (note.txHash) {
        if (!notesByTx[note.txHash]) {
          notesByTx[note.txHash] = []
        }
        notesByTx[note.txHash].push(note)
      }
    }

    // Process notes transactions
    for (const [txHash, txNotes] of Object.entries(notesByTx)) {
      const spent = txNotes.filter((n) => n.spent)
      const unspent = txNotes.filter((n) => !n.spent)

      if (spent.length > 0 && unspent.length > 0) {
        // Swap: has both input (spent) and output (unspent/new) notes
        const inputNote = spent[0]
        const outputNote = unspent[0]

        const inputDecimals = inputNote.decimals ?? 7
        const outputDecimals = outputNote.decimals ?? 7

        const amountInVal = baseUnitsToNumber(BigInt(inputNote.amount), inputDecimals)
        const amountOutVal = baseUnitsToNumber(BigInt(outputNote.amount), outputDecimals)

        items.push({
          id: txHash,
          type: 'Swap',
          pairOrAsset: `${inputNote.assetCode} → ${outputNote.assetCode}`,
          amountIn: formatAmount(amountInVal),
          amountOut: formatAmount(amountOutVal),
          txHash,
          createdAt: outputNote.createdAt,
        })
      } else if (unspent.length > 0) {
        // Deposit: only has new incoming notes
        const note = unspent[0]
        const decimals = note.decimals ?? 7
        const amountVal = baseUnitsToNumber(BigInt(note.amount), decimals)

        items.push({
          id: txHash,
          type: 'Deposit',
          pairOrAsset: note.assetCode,
          amountOut: formatAmount(amountVal),
          txHash,
          createdAt: note.createdAt,
        })
      } else if (spent.length > 0) {
        // Withdrawal: only has spent notes
        const note = spent[0]
        const decimals = note.decimals ?? 7
        const amountVal = baseUnitsToNumber(BigInt(note.amount), decimals)
        ...
        items.push({
          ...
          type: 'Withdrawal',
          pairOrAsset: note.assetCode,
          amountIn: formatAmount(amountVal),
          txHash,
          createdAt: note.createdAt,
        })
      }
    ...
    for (const order of orders) {
      if (order.status !== 'open') {
        const type = order.status === 'filled' ? 'Limit Order (Filled)' : 'Limit Order (Cancelled)'
        const amountNum = baseUnitsToNumber(BigInt(order.amount), order.lockedDecimals)
        const sideText = order.side === 0 ? 'Buy' : 'Sell'

        items.push({
          id: order.commitment,
          type: type as any,
          pairOrAsset: `${order.baseCode}/${order.quoteCode} (${sideText})`,
          amountIn: order.side === 1 ? formatAmount(amountNum) : undefined,
          amountOut: order.side === 0 ? formatAmount(amountNum) : undefined,
          txHash: order.txHash,
  ...
  async swapShielded({ assetIn, assetOut, amountIn, amountOutMin, path, deadline }: SwapShieldedParams): Promise<TxResult> {
    if (!SWAP_ROUTER_CONTRACT_ID) {
      throw new Error('LarelSwapRouter contract ID is not configured.')
    }
    const notes = loadNotes().filter((n) => !n.spent && n.assetCode === assetIn && n.leafIndex !== undefined)
    const decimalsIn = notes.find((n) => n.decimals !== undefined)?.decimals ?? assetMeta(assetIn).decimals
    const decimalsOut = assetMeta(assetOut).decimals

    const amountInBase = toBaseUnits(amountIn, decimalsIn)
    const amountOutMinBase = toBaseUnits(amountOutMin, decimalsOut)

    if (amountInBase <= 0n) throw new Error('Amount must be greater than zero.')

    const chosen = notes
      .filter((n) => BigInt(n.amount) >= amountInBase)
      .sort((a, b) => (BigInt(a.amount) < BigInt(b.amount) ? -1 : BigInt(a.amount) > BigInt(b.amount) ? 1 : 0))[0]
    if (!chosen) {
      throw new Error(`No single shielded ${assetIn} note covers this amount.`)
    }

    const input = toBalanceNote(chosen)
    const witness = await this.spendWitness(chosen)
    const noteAmount = input.amount // The entire note's amount

    // Scale the minimum output proportionally to match the full note amount being swapped
    const scaledAmountOutMinBase = (noteAmount * amountOutMinBase) / amountInBase

    const spendingKey = getSpendingKey()
    const rHash = recipientHash(SWAP_ROUTER_CONTRACT_ID)
    ...
    // Compute target output note commitment
    const targetSac = assetOut === 'XLM' ? NATIVE_SAC : assetMeta(assetOut).sac
    const targetAssetId = assetIdFromAddress(targetSac)
    const targetAssetIdHex = fieldToHex(targetAssetId)

    // Deterministic blinding so this swap output note is recoverable and verifiable by the indexer
    const salt = loadNotes().filter(
      (n) => n.source === 'deposit' && n.assetId === targetAssetIdHex && BigInt(n.amount) === scaledAmountOutMinBase,
    ).length
    const outputNote = createNote({
      assetId: targetAssetId,
      amount: scaledAmountOutMinBase,
      spendingKey,
      blinding: depositBlinding(noteSecret(spendingKey), targetAssetId, scaledAmountOutMinBase, salt),
    })
    outputNote.assetAddress = targetSac

    const swapRouter = new LarelSwapRouterContract({
      contractId: SWAP_ROUTER_CONTRACT_ID,
      networkPassphrase: NETWORK_PASSPHRASE,
    })

    const from = await this.requireAddress()
    const resolvedDeadline = deadline ? BigInt(deadline) : BigInt(Math.floor(Date.now() / 1000) + 1200)

    const op = swapRouter.swapShieldedOp({
      pool: POOL_CONTRACT_ID,
      soroswapRouter: SOROSWAP_ROUTER_ADDRESS,
      proof: proof.proof,
      publicInputs: encodePublicInputs(proof.publicInputs),
      amountIn: noteAmount,
      tokenIn: input.assetAddress ?? NATIVE_SAC,
      tokenOut: outputNote.assetAddress ?? NATIVE_SAC,
      amountOutMin: scaledAmountOutMinBase,
      path,
      deadline: resolvedDeadline,
      recipientCommitment: outputNote.commitment,
      userPublicAddress: from,
    })
    ...
    markSpent(chosen.commitment)
    addNote(outputNote, { assetCode: assetOut, txHash: hash, decimals: decimalsOut, source: 'deposit' })
    void syncIndexer()
