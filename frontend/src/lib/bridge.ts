 ...
 *
 *   • EVM side  — `LarelBridgeL1.lock` via wagmi/viem (MetaMask). Native ETH locks
 *                 `value = amount`; ERC20 locks `approve` + `lock` (BRIDGE_SPEC §4).
 *   • Stellar   — reads the `EthLightClient` trusted head and the `LarelBridge`
 ...
 *
 * A "bridge in" creates a Larel note with a *bridged* `asset_id` (BRIDGE_SPEC §3),
 ...
 */
import { buildTransaction, createNote, fieldToHex, type BalanceNote, type Field } from '@larel/sdk'
...
export interface BridgeToken {
  ...
  native: boolean
  /** The Larel shielded `AssetCode` this bridges into. */
  assetCode: AssetCode
}
...
// ---------------------------------------------------------------------------
// ABIs (inlined — matches bridge/l1/src/LarelBridgeL1.sol)
// ---------------------------------------------------------------------------

export const LAREL_BRIDGE_L1_ABI = [
  {
    type: 'function',
    name: 'lock',
...
/**
 * Create the Larel note that backs a bridge-in. The note's `asset_id` is the bridged
...
/**
 * Write `LarelBridgeL1.lock(commitment, token, amount)` on Sepolia.
...
export async function lockOnL1(params: {
  ...
  if (!l1Configured()) {
    throw new Error(
      'L1 bridge address is not configured. Set VITE_L1_BRIDGE_ADDRESS to the deployed LarelBridgeL1, or use mock mode (VITE_USE_MOCK).',
    )
  }
...
/** Has the `LarelBridge` already minted this inbound commitment? (BRIDGE_SPEC §7/§12). */
export async function readIsBridged(commitment: Hex): Promise<boolean | null> {
  if (!LAREL_BRIDGE_ID) return null
  const arg = xdr.ScVal.scvBytes(Buffer.from(hexToBytes(commitment)))
  const native = await simulateRead(LAREL_BRIDGE_ID, 'is_bridged', [arg])
...
export { LAREL_BRIDGE_ID, ETH_LIGHT_CLIENT_ID, L1_BRIDGE_ADDRESS }
