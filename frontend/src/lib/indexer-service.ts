 ...
 *
 * Holds one live {@link LarelIndexer} per shielded identity. On start it hydrates the
...
import { fieldToHex, type Field } from '@larel/sdk'
import { POOL_DEPLOY_LEDGER, SOROBAN_RPC_URL } from './config'
import { LarelIndexer, identityFromSpendingKey, type SyncStats } from './indexer'
import { loadIndexCursor, loadLeaves, loadNotes, saveIndexCursor, saveLeaves } from './note-store'

let active: { key: string; indexer: LarelIndexer } | null = null
 ...
 */
export function startIndexer(spendingKey: Field): LarelIndexer {
  const key = fieldToHex(spendingKey)
  if (active?.key === key) return active.indexer
  const indexer = new LarelIndexer(identityFromSpendingKey(spendingKey))
...
export function getIndexer(): LarelIndexer | null {
