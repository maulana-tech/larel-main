/**
 * Larel orchestrator: the deterministic (non-proof) paths — deposit, local-tree sync,
 * balance/ order views, and the precise "no prover configured" errors for proof flows.
 */
import { describe, expect, it } from "vitest";
import { Larel, nativeAsset } from "../src/index.js";
...
const newSdk = () => new Larel({ contractId: CONTRACT_ID, spendingKey: 42n });
...
describe("proof flows require a configured prover", () => {
  ...
  it("swapShielded assembles full padded inputs (then needs a prover)", async () => {
    const sdk = newSdk();
    const { note } = sdk.deposit({ asset: XLM, amount: 1000n, from: ACCOUNT });
    sdk.observeCommitment(note.commitment);
    ...
    await expect(
      sdk.swapShielded({
        swapRouterContractId: "CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABSC4",
        soroswapRouterAddress: "CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAABSC4",
        note,
        tokenInAddress: "CDLZFC3SYJYDZT7K67VZ75HPJVIEUVNIXF47ZG2FB2RMQQVU2HHGCYSC",
        tokenOutAddress: "CB3TLW74NBIOT3BUWOZ3TUM6RFDF6A4GVIRUQRQZABG5KPOUL4JJOV2F",
        amountIn: 1000n,
        amountOutMin: 900n,
