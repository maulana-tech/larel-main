...
const scvU64 = (n: bigint): xdr.ScVal => nativeToScVal(n, { type: "u64" });
const scvField = (f: Field): xdr.ScVal => scvBytes(fieldToBytes(f));

/**
 * Builds Soroban invoke operations against a deployed LarelPool contract. The returned
...
/**
 * Builds Soroban invoke operations against a deployed LarelSwapRouter contract.
 */
export class LarelSwapRouterContract {
  readonly contract: Contract;
  readonly networkPassphrase: string;
  ...
  swapShieldedOp(args: {
    ...
    return this.contract.call(
      "swap_shielded",
      scvAddress(args.pool),
      scvAddress(args.soroswapRouter),
      scvBytes(args.proof),
      scvBytes(args.publicInputs),
      scvI128(args.amountIn),
      scvAddress(args.tokenIn),
      scvAddress(args.tokenOut),
      scvI128(args.amountOutMin),
      xdr.ScVal.scvVec(args.path.map(scvAddress)),
      scvU64(args.deadline),
      scvField(args.recipientCommitment),
      scvAddress(args.userPublicAddress)
    );
  }
}
