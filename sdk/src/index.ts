/**
 * @larel/sdk — TypeScript client for the Larel privacy platform on Stellar.
 *
 * Re-exports every module plus a {@link Larel} orchestrator implementing the
 * {@link LarelSdk} surface the frontend expects. All cryptographic invariants
...
import { LarelContract, LarelSwapRouterContract, encodePublicInputs, recipientHash, type CircuitName, assetIdFromAddress } from "./stellar.js";
 ...
 * {@link LarelConfig.provers}); the deterministic crypto/encoding around them runs
 * regardless.
 */
...
/** Construction options for {@link Larel}. */
export interface LarelConfig {
  ...
  /** Local mirror of the on-chain Merkle tree (kept in sync via {@link Larel.observeCommitment}). */
  tree?: MerkleTree;
  /** Pre-seeded wallet. */
  wallet?: Wallet;
}
...
export class Larel implements LarelSdk {
  ...
  constructor(config: LarelConfig) {
    ...
    this.contract = new LarelContract({
  ...
  async swapShielded(params: {
    ...
    const merkle = this.merkleProofFor(note);
    const nullifier = noteNullifier(note);
    const rHash = recipientHash(params.swapRouterContractId);
    ...
    const outputNote = createNote({
      assetId: assetIdFromAddress(params.tokenOutAddress),
      amount: params.amountOutMin,
      spendingKey: this.keys.spendingKey,
    });
    outputNote.assetAddress = params.tokenOutAddress;

    const routerContract = new LarelSwapRouterContract({
      contractId: params.swapRouterContractId,
      networkPassphrase: this.contract.networkPassphrase,
    });

    const operation = routerContract.swapShieldedOp({
      pool: this.contract.contract.contractId(),
      soroswapRouter: params.soroswapRouterAddress,
      proof: proof.proof,
      publicInputs: encodePublicInputs(proof.publicInputs),
      ...
      userPublicAddress: params.userPublicAddress,
    });

    this.wallet.markNoteSpent(note);
    this.wallet.addNote(outputNote);

    return { operation, proof, nullifiers: [nullifier], outputNotes: [outputNote] };
  }
  ...
  private async prove(circuit: CircuitName, inputs: CircuitInputMap): Promise<ProofData> {
    ...
    if (!prover) {
      throw new Error(
        `no prover configured for circuit "${circuit}". Provide a compiled circuit via LarelConfig.provers (pending feat/circuits integration).`,
      );
    }
